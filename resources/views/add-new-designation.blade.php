...
@section("title")
    ...
    <title>Add Designation | {{ env('MY_SITE_NAME') }}</title>
        ...
        <div class="col-lg-12">
          ...
          <div class="card">
            <div class="card-body">
              ...
              <form method="post" action="{{ route('save-new-designation')}}" class="row g-3 needs-validation" novalidate>
                @csrf
                ...
                <div class="col-md-6 position-relative">
                  ...
                  <select class="form-select" name="company_id" id="company_id" required>
                    ...
                    <option value="{{$company_detail['id']}}" @if(old('company_id')==$company_detail['id']) selected @endif>{{$company_detail['name']}}</option>
                    @endforeach
                  </select>
